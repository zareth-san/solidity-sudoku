...
contract ContractTest is DSTest {
    ...
    function setUp() public {
        sudokuChallenge = new SudokuChallenge(testCorrectInput());
        sudokuExchange = new SudokuExchange();
        rewardToken = new ERC20("rewardToken", "RT");
        //rewardToken._mint(account, amount);
    }

    function testCorrectInput() public returns (uint8[81] memory){
        uint8[81] memory challenge = 
            [
            3, 1, 6, 5, 7, 8, 4, 9, 2,
            5, 2, 9, 1, 3, 4, 7, 6, 8,
            4, 8, 7, 6, 2, 9, 5, 3, 1,
    ...
    function testChallange() public returns (uint8[81] memory){
        uint8[81] memory challenge = 
            [
            3, 0, 6, 5, 0, 8, 4, 0, 0,
            5, 2, 0, 0, 0, 0, 0, 0, 0,
